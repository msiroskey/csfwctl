...
| `file_path` | string (glob, ≤999 chars)         | no       | Executable-filepath glob (e.g. `C:\Program Files\app\*.exe`). Rule matches only traffic from a process whose image path matches. Windows-oriented (CrowdStrike's application-aware match). Rides in the API `fields` array. |
