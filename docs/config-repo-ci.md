...
typically blocked by a corporate firewall from a Docker container.
Instead, capture the host key once from a machine that can reach the
server and store it as a CI variable so the pipeline never needs to
scan at runtime.
...
| `GITLAB_SSH_KNOWN_HOSTS` | All | No | Output of `ssh-keyscan -H code.osu.edu` from an admin workstation. Avoids a runtime port 22 connection from the Docker container, which is firewall-blocked. |
...
.csfwctl-job:
  ...
  cache:
    ...
    paths:
      - .cache/pip/
  before_script:
    # git and openssh-client are not present in python:3.11-slim
    - apt-get update -qq && apt-get install -y git openssh-client --no-install-recommends -qq
    - |
